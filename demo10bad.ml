...
class [ 'self ] map = object ( _ : 'self )
  method private visit_option : 
    'a 'b . ( 'env -> 'a -> 'b ) -> 'env -> 'a option -> 'b option 
    = fun f env -> function
      | None -> None | Some x -> Some (f env x) (** 2 **)
end

class virtual [ 'self ] fold = object (self : 'self)
  (* There is a similarity between 1 and 2 so we define visit_option in `fold` like this *)
  method private visit_option : 'a . ('env -> 'a -> 'r) -> 'env -> 'a option -> 's 
  (* We assign the most general type that is polym. over `'a` abd can't be polymorphic
     over `'r` and over `'s` because they appear in another methods. They will be 
     implicitly quantified on level of class *)
    = fun f env ox ->
      match ox with
